...
def get_nearby_attractions(lat: float, lon: float, radius: int = 5000, limit: int = 10):
    try:
        url = (
            ...
            f"radius={radius}&lon={lon}&lat={lat}&limit={limit}&apikey={OPENTRIPMAP_API_KEY}&rate=2&format=json"
        )
        res = requests.get(url, timeout=20)
        res.raise_for_status()
...
def get_global_city_context(city_name: str):
    ...
    return {"coordinates": coords, "attractions": attractions}
...
def generate_zephyr_response(prompt: str, max_tokens: int = 350) -> str:
    ...
    try:
        ...
        # Return error string instead of raising 500
